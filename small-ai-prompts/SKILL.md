...
Total: 6,500 unique YPO AI prompts. Each prompt is stored once but keeps its full `Categories:` tag line — so a search for any category (e.g. "sales") still surfaces every prompt tagged with it, regardless of which file it lives in. Think of it like a library where each book sits on one shelf but the catalog finds it under every subject it covers. (This desktop build packs the prompts into ~120 files to stay within the app's file and size limits; content is identical to the full library.)
